# -*- coding: utf-8 -*-
"""
Created on Sat Feb 24 13:17:15 2018
...
def model(b, u_):
	...
	with tf.variable_scope('Restriction1') as scope:
		...
		restrict_1 = conv_restrict(b1, weights, biases, [1,1,2,1], "VALID", g1) #IS THIS CORRECT STRIDE?
		
	with tf.variable_scope('Restriction2') as scope:
		weights = tf.Variable(tf.random_normal([g2,1,1]), name='R1')
		biases = tf.Variable(tf.zeros(1),name='b_R1')
		restrict_2 = tf.reshape(conv_restrict(restrict_1, weights, biases, [1,1,2,1], "SAME", g2), [batch_size, 1, g2]) #IS THIS CORRECT STRIDE?

	with tf.variable_scope('Inverse') as scope:
		weights3 = tf.Variable(tf.random_normal([batch_size, g2,g2]), name='A0inv')
		biases3 = tf.Variable(tf.zeros(g2),name='b_A0inv')
		h0 = tf.identity(tf.matmul(restrict_2, weights3)+biases3)
		h0 = tf.reshape(h0, [batch_size,g2,1])

	with tf.variable_scope('Prolongation1') as scope:
		weights = tf.Variable(tf.random_normal([g2,1,1]), name='P1')
